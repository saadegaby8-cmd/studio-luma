...
DEFAULT_SETTINGS: Dict[str, Any] = {
    ...
    "flux_tryon_model": "fal-ai/bytedance/seedream/v4.5/edit",   # avatar + prenda (FLUX.2 dev: calidad + moder. liviana)
    "flux_edit_model": "fal-ai/bytedance/seedream/v4.5/edit",    # sin avatar / solo producto (dev, multi-referencia)
    "flux_fallback_model": "fal-ai/flux-2-lora-gallery/virtual-tryon",  # permisivo, último recurso si dev bloquea
    "precio_flux": 0.04,                # US$ por imagen con FLUX (editable)
    ...
    if mode == "on_model":
        ...
        if use_flux or _flux_on_block:
            ...
            flux_slug = str((settings.get("flux_tryon_model") if persona_b64
                             else settings.get("flux_edit_model")) or "fal-ai/bytedance/seedream/v4.5/edit")
            if use_flux:
                parts = flux_parts
                prompt = _fprompt
                note = "FLUX · " + note
        ...
        if use_flux:
            ...
            prompt = build_prompt_product_only(params, settings, modo_p, paneles, aspect,
                                               n_prod)
            parts = [{"text": prompt}] + [_img_part(b) for b in prod_b64s]
            flux_slug = str(settings.get("flux_edit_model") or "fal-ai/bytedance/seedream/v4.5/edit")
            note = "FLUX · " + note
    elif mode == "trio":
